...
[5] 初始步长的选择是自动的，由求解器根据 rtol/atol 自适应调整：
    在 torchdiffeq(以及 Scipy、MATLAB)中，初始步长 h0 绝不是拍脑袋定的一个常数(比如 0.01),而是通过一套严谨的启发式算法, 根据你的输入数据算出来的。
[6] 从头开始训练
"""
...
class Config:
    ...
    # ---- Checkpoint Periodic Save (新增：按要求) ----
    SAVE_EVERY_EPOCHS: int = 50  # 每 50 轮额外保存一次

    # ---- ODE Solver (固定 dopri5 + 自适应步长) ----
    METHOD: str = "dopri5"
    RTOL: float = 1e-3
    ATOL: float = 1e-4
    # adjoint 反传的容差（可与 forward 相同；想加速可放宽）
...
def get_current_lr(optimizer: torch.optim.Optimizer) -> float:
    # 取第一个 param group 的 lr 作为展示（多组也会统一调整）
...
# =========================
# (2) ODE Function: dx/dt = f(t, x; θ)，它定义了状态 x 随时间 t 的导数：
# =========================
...
class ODEBlock(nn.Module):
    ...
    def forward(self, x):
        # reset NFE each forward
        self.odefunc.nfe = 0

        # 时间张量 dtype/device 与 x 对齐
        ...
        out = odeint(
            ...
            x,
            t,
            method=self.cfg.METHOD,      # 固定 dopri5
            rtol=self.cfg.RTOL,
            atol=self.cfg.ATOL,
            options=options,
            adjoint_rtol=self.cfg.ADJOINT_RTOL,
            adjoint_atol=self.cfg.ADJOINT_ATOL,
            adjoint_method=self.cfg.METHOD,  # adjoint 也用 dopri5
...
def main():
    ...
    # ---- 动态学习率状态（新增：按要求）----
    best_monitored_loss = float("inf")  # 监控 test loss 的“历史最小值”
    bad_epochs = 0                      # 连续未下降轮数
    ...
    for epoch in range(1, CFG.EPOCHS + 1):
        ...
        # =========================
        # (A) 动态学习率：连续 15 轮 loss 不下降则 lr 减半（新增：按要求）
        # 监控指标：te_loss（test/eval loss）
        # =========================
        lr_reduced = False
        ...
        # 打印：增加当前 lr + 动态 lr 状态（不影响原策略，仅增加信息）
